...
const size_t _num_spikespace = 151;
 ...
 const double _lio_1 = (- 52.0) * 0.001;
    ...
    long _count = 0;
    for(size_t _idx=0; _idx<150; _idx++)
    {
        const size_t _vectorisation_idx = _idx;
                
        const double v = _ptr_array_Regular_Spiking_Interneurons_v[_idx];
        const char _cond = v > _lio_1;

        if(_cond) {
            _ptr_array_Regular_Spiking_Interneurons__spikespace[_count++] = _idx;
        }
    }
    _ptr_array_Regular_Spiking_Interneurons__spikespace[150] = _count;
