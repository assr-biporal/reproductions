...
void _run_Pyramidal_Cells_thresholder_codeobject()
{
...
const size_t _num_spikespace = 801;
 ...
 const double _lio_1 = (- 52.0) * 0.001;
    ...
    long _count = 0;
    for(size_t _idx=0; _idx<800; _idx++)
    {
        const size_t _vectorisation_idx = _idx;
                
        const double v = _ptr_array_Pyramidal_Cells_v[_idx];
        const char _cond = v > _lio_1;

        if(_cond) {
            _ptr_array_Pyramidal_Cells__spikespace[_count++] = _idx;
        }
    }
    _ptr_array_Pyramidal_Cells__spikespace[800] = _count;
